...
import '../../page/P02SUMMARY/P02SUMMARYMAIN.dart';
...
abstract class P02SUMMARYGETDATA_Event {}

class P02SUMMARYGETDATA_GET extends P02SUMMARYGETDATA_Event {}

class P02SUMMARYGETDATA_GET2 extends P02SUMMARYGETDATA_Event {}

class P02SUMMARYGETDATA_GET3 extends P02SUMMARYGETDATA_Event {}

class P02SUMMARYGETDATA_FLUSH extends P02SUMMARYGETDATA_Event {}

class P02SUMMARYGETDATA_Bloc extends Bloc<P02SUMMARYGETDATA_Event, List<P02SUMMARYGETDATAclass>> {
  P02SUMMARYGETDATA_Bloc() : super([]) {
    on<P02SUMMARYGETDATA_GET>((event, emit) {
      return _P02SUMMARYGETDATA_GET([], emit);
    });
/* 
    on<P02SUMMARYGETDATA_GET2>((event, emit) {
      return _P02SUMMARYGETDATA_GET2([], emit);
    });
    on<P02SUMMARYGETDATA_GET3>((event, emit) {
      return _P02SUMMARYGETDATA_GET3([], emit);
    });
    on<P02SUMMARYGETDATA_FLUSH>((event, emit) {
      return _P02SUMMARYGETDATA_FLUSH([], emit);
    }); */
  }

  Future<void> _P02SUMMARYGETDATA_GET(
      List<P02SUMMARYGETDATAclass> toAdd, Emitter<List<P02SUMMARYGETDATAclass>> emit) async {
    // FreeLoadingTan(P02SUMMARYcontext);
    List<P02SUMMARYGETDATAclass> output = [];
    //-------------------------------------------------------------------------------------
    try {
      ...
      if (response.statusCode == 200) {
        ...
        input = databuff;
        // print(input);
        List<P02SUMMARYGETDATAclass> outputdata = input.map((data) {
          return P02SUMMARYGETDATAclass(
            id: savenull(data['id']),
        ...
        // Navigator.pop(P01INPUTDATAcontext);
        showErrorPopup(P02SUMMARYcontext, response.toString());
        output = [];
        emit(output);
      }
      ...
      if (e.type == DioExceptionType.sendTimeout) {
        showErrorPopup(P02SUMMARYcontext, "Send timeout");
      } else if (e.type == DioExceptionType.receiveTimeout) {
        showErrorPopup(P02SUMMARYcontext, "Receive timeout");
      } else {
        showErrorPopup(P02SUMMARYcontext, e.message ?? "Unknown Dio error");
      }
    } catch (e) {
      // Navigator.pop(P01INPUTDATAcontext);
      showErrorPopup(P02SUMMARYcontext, e.toString());
    }
  }
}

class P02SUMMARYGETDATAclass {
  P02SUMMARYGETDATAclass({
    this.id = '',
    this.po_no = '',
    this.customer_id = '',
    this.custfull = '',
